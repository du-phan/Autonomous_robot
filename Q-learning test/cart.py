...
def cart_pole_with_qlearning():
    ...
    cart_position_bins = pd.cut([-4.8, 4.8], bins=10, retbins=True)[1][1:-1]
    pole_angle_bins = pd.cut([-.41, .41], bins=10, retbins=True)[1][1:-1]
    ...
    for episode in xrange(10000): # number of training iterations

        if episode % 100 == 0:
            print "Episode {}, average iteration = {}. ".format(episode, last_time_steps.mean())
            
            
        
        #env.render()
        observation = env.reset()
        cart_position, cart_velocity, pole_angle, angle_rate = observation
        state = build_state([to_bin(cart_position, cart_position_bins),
                             to_bin(cart_velocity, cart_velocity_bins),
                             to_bin(pole_angle, pole_angle_bins),
                             to_bin(angle_rate, angle_rate_bins)])
