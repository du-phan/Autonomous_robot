...
void i2c_start(Port port) {
    digitalWrite(port.scl,HIGH); // HAHN, added
    digitalWrite(port.sda,HIGH); // HAHN, added
    I2C_DELAY;
...
void i2c_stop(Port port) {
    ...
    I2C_DELAY; // HAHN, to ensure the stop bit is received, before next transmission
}

boolean i2c_get_ack(Port port) {

    digitalWrite(port.sda,HIGH); // HAHN, to let the slave drive the bus
    digitalWrite(port.scl,LOW); // HAHN, scl should already be LOW by now
    I2C_DELAY;
    digitalWrite(port.scl,HIGH);
    ...
    if (!digitalRead(port.sda)) {
      I2C_DELAY;
      digitalWrite(port.scl,LOW);
	  digitalWrite(port.sda,HIGH); // HAHN, unnecessary
      return true;
    } else {
      I2C_DELAY;
      digitalWrite(port.scl,LOW);
...
void i2c_write_ack(Port port, boolean nack=false) {
    digitalWrite(port.sda,nack); // HAHN, ACK is active LOW!!
    I2C_DELAY;
    digitalWrite(port.scl,HIGH);
    I2C_DELAY;
	digitalWrite(port.scl,LOW);
...
uint8 i2c_shift_in(Port port) {
    ...
    for (i=0;i<8;i++) {
        I2C_DELAY;
        data += digitalRead(port.sda) << (7-i); // HAHN, the value should be read at the rising edge
        digitalWrite(port.scl,HIGH);
        I2C_DELAY;
        digitalWrite(port.scl,LOW);
    }
...
void i2c_shift_out(Port port, uint8 val) {
    ...
    for (i=0;i<8;i++) {
        digitalWrite(port.sda, !!(val & (1 << (7 - i))));  // HAHN, the value should be set at falling edge
        I2C_DELAY;
        digitalWrite(port.scl, HIGH); // HAHN, the value is read at rising edge
        I2C_DELAY;
        digitalWrite(port.scl, LOW);
    }
}
...
uint8 TwoWire::requestFrom(uint8 address, int num_bytes) {
    ...
    while (rx_buf_len < num_bytes) {
        *(rx_buf + rx_buf_len++) = i2c_shift_in(port);
        i2c_write_ack(port, rx_buf_len == num_bytes);
    }
...
void TwoWire::setSkipStop(uint8 skipStop) {
